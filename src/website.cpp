...
volatile bool     req_scan      = false;
volatile uint32_t req_scanMs    = 0;
volatile bool     req_stop      = false;
char              req_capLabel[12] = "";
volatile bool     req_replay    = false;
char              req_replayLbl[12] = "";
  ...
  <details id="mqttSection" style="background:#0a0a0a;border:1px solid #333;padding:10px 14px;border-radius:3px;margin:10px 0">
    ...
    <div style="margin-top:10px">
      ...
      <div class="row" style="margin-top:8px">
        ...
        <button style="font-size:0.85em;padding:4px 10px;background:#600;color:#fff;border:none;border-radius:3px;cursor:pointer;margin-left:4px" onclick="resetWifi()">Reset WiFi</button>
...
<script>
  ...
  async function resetWifi() {
    if (!confirm('Clear saved WiFi credentials and reboot into setup portal?')) return;
    await fetch('/resetwifi', {method:'POST'}).then(r=>r.text()).then(t=>alert(t)).catch(()=>{});
  }
...
void websiteInit() {
    ...
    server.on("/packets", HTTP_GET, [](AsyncWebServerRequest *r) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < (pktMeta[i].bitCount + 7) / 8 && j < 8; j++)
                snprintf(bmc + j*2, 3, "%02X", pktMeta[i].bits[j]);
            char hdr[160];
            snprintf(hdr, sizeof(hdr),
                "{\"n\":%d,\"ts\":%d,\"tl\":%d,\"reps\":%d,\"sof\":%d,\"bn\":%d"
                ",\"bmc\":\"%s\",\"lbl\":\"%s\",\"saved\":%s,\"p\":[",
                pktLen[i], pktMeta[i].tShort, pktMeta[i].tLong,
                pktMeta[i].reps, pktMeta[i].sofCount, pktMeta[i].bitCount,
                bmc, pktMeta[i].label, pktMeta[i].saved ? "true" : "false");
            resp->print(hdr);
            for (int j = 0; j < pktLen[i]; j++) {
                if (j) resp->print(",");
                resp->print(pktData[i][j]);
            }
            resp->print("]}");
        }
        resp->print("]");
    ...
    server.on("/savesig", HTTP_GET, [](AsyncWebServerRequest *r) {
        ...
        int8_t slot = findBestBMCSlot(lbl.c_str());
    ...
    server.on("/mqttcfg", HTTP_GET, [](AsyncWebServerRequest *r) {
        ...
        snprintf(buf, sizeof(buf),
            ...
            mqttIsConnected() ? "true" : "false", devId, devName);
        r->send(200, "application/json", buf);
    });
    ...
    server.on("/mqttdiscover", HTTP_GET, [](AsyncWebServerRequest *r) {
        ...
        r->send(200, "text/plain", mqttIsConnected() ? "published" : "not connected");
    });

    server.on("/resetwifi", HTTP_POST, [](AsyncWebServerRequest *r) {
        wifiClearCredentials();
        r->send(200, "text/plain", "WiFi credentials cleared \xe2\x80\x94 rebooting\xe2\x80\xa6");
        req_restart = true;
